describe('Compra de Produtos', () => {
  it('Caso 1 - Usuário realiza compra com sucesso', () => {
    ...
    cy.get('[data-test="shopping-cart-badge"]').should('be.visible').and('have.text', '2');  //Verifica quantidade itens
    ...
    cy.get('[data-test="payment-info-label"]').should('be.visible').and('have.text', 'Payment Information:');
    cy.get('[data-test="payment-info-value"]').should('be.visible').and('have.text', 'SauceCard #31337');
    //Verifica Informação de Envio
    cy.get('[data-test="shipping-info-label"]').should('be.visible').and('have.text', 'Shipping Information:');
    cy.get('[data-test="shipping-info-value"]').should('be.visible').and('have.text', 'Free Pony Express Delivery!');
    //Verifica o Valor Total e Taxa
    cy.get('[data-test="total-info-label"]').should('be.visible').and('have.text', 'Price Total');
    cy.get('[data-test="subtotal-label"]').should('be.visible').and('have.text', 'Item total: $79.98');
    cy.get('[data-test="tax-label"]').should('be.visible').and('have.text', 'Tax: $6.40');
    cy.get('[data-test="total-label"]').should('be.visible').and('have.text', 'Total: $86.38');
